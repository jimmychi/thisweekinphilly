...
const NEIGHBORHOODS = ["All", "Center City", "Old City", "Fishtown", "Rittenhouse", "South Philly", "Northern Liberties", "Manayunk", "University City"];
const PRICE = { 1: "$", 2: "$$", 3: "$$$", 4: "$$$$" };
...
const styles = {
  ...
  filters: { background: "var(--warm-white)", borderBottom: "1px solid var(--stone)", padding: "12px 16px", display: "flex", gap: 6, flexWrap: "nowrap", overflowX: "auto", WebkitOverflowScrolling: "touch", position: "sticky", top: 68, zIndex: 90, backdropFilter: "blur(8px)", justifyContent: "center" },
  ...
  cardMeta: { display: "flex", gap: 8, alignItems: "center", marginBottom: 6, flexWrap: "wrap" },
  rating: { background: "var(--brass)", color: "#fff", borderRadius: 4, padding: "2px 7px", fontSize: "0.8rem", fontWeight: 700 },
  price: { color: "var(--brick)", fontWeight: 700, fontSize: "0.85rem" },
  openNow: { color: "#2e7d32", fontSize: "0.78rem", fontWeight: 600 },
  address: { fontSize: "0.82rem", color: "var(--stone-dark)", fontFamily: "var(--font-body)" },
...
export default function Bars() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/restaurants/bars${n}`)
      .then(r => r.json())
      .then(data => { setBars(data.restaurants || []); setLoading(false); })
    ...
    <div style={styles.page}>
      ...
      <div style={styles.header}>
        <div style={styles.headerInner}>
          ...
          <p style={styles.sub}>Neighborhood Bars & Cafes</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={styles.grid}>
          {bars.map(r => (
            <div key={r.id} style={styles.card} onClick={() => navigate(`/restaurants/${encodeURIComponent(r.id)}`)}>
              {r.image ? <img src={r.image} alt={r.name} style={styles.cardImg} /> : <div style={styles.cardImgPlaceholder}>🍺</div>}
