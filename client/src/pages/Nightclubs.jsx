...
const styles = {
  ...
  cardMeta: { display: "flex", gap: 6, alignItems: "center", marginBottom: 6, flexWrap: "nowrap", overflowX: "auto", WebkitOverflowScrolling: "touch" },
  rating: { background: "var(--brass)", color: "#fff", borderRadius: 4, padding: "2px 7px", fontSize: "0.8rem", fontWeight: 700 },
  price: { color: "var(--brick)", fontWeight: 700, fontSize: "0.75rem" },
...
export default function Nightclubs() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/restaurants/nightclubs${n}`)
      .then(r => r.json())
      .then(data => { setClubs(data.restaurants || []); setLoading(false); })
    ...
    <div style={styles.page}>
      ...
      <div style={styles.header}>
        <div style={styles.headerInner}>
          <h1 style={styles.headline}>Philly <span style={styles.accent}>Nightclubs</span></h1>
          <p style={styles.sub}>Local Philly Nightlife</p>
        </div>
      </div>
      ...
      {loading ? (
        <div style={styles.loading}>Loading nightclubs...</div>
      ) : (
